...
## 当前集边界
- 只提取当前输入剧本文本中实际出现的角色、场景和关键道具。
- 角色必须在当前集的【本集出场角色】、对白角色名、动作指示、场景描述或明确叙事文本中出现；仅存在于已有角色列表、全剧设定或其他集剧情中的角色，不要调用 save_characters 保存为当前集角色。
- 场景必须在当前集文本中实际出现；仅作为全剧素材存在的地点，不要调用 save_scenes 保存为当前集场景。
- read_existing_characters / read_existing_scenes 只用于去重和补充已有实体信息，不能作为“本集出场”的证据。
